...
description: 포켓몬·맵 에셋(픽셀 전용)을 가져오고 배치하는 스킬. PokeAPI·Another Red·pokemondb·Spriters·Bulbagarden, front/back/icon 스프라이트, tileset, animated gif, npm run fetch, pokemonSprite 로더, CORS, public/assets 경로, 팔레트·폰트·SFX 소스비교. 트리거 예: "스프라이트 적용해줘", "에셋 가져와", "타일셋 추가", "도감 받아", "더 좋은 소스 비교해줘", "OO 후보 뽑아와".
---
...
2. **Another Red (로컬 원본, 예: `D:/Pokemon Another Red_PWT_250829/` — ⚠️PC마다 위치 다름)** — 9세대 픽셀 핵심. `Graphics/`만 사용(Data는 RPG Maker 바이너리라 못 씀). 추가 복사: `node tools/import-from-anotherred.mjs "<그 PC의 AR경로>" <back|followers|trainers|battlebacks|ui>`.
