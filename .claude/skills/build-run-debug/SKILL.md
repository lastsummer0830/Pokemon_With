...
## 📸 화면 캡처(스크린샷 검증) — 반드시 이 방식 (안 지키면 헛발질)
- 이 게임은 Phaser WebGL 캔버스라 page.screenshot()(Playwright)는 게임 화면이 검게 저장된다(메뉴 텍스트만 남음). 이걸로 "봤다/똑같다" 판단 절대 금지 — 지난 몇 주 헛발질의 원인.
- 올바른 방식 = headed 브라우저 + 게임 렌더러 스냅샷. 템플릿: tools/shot-menuA.mjs 의 snap(page, path) (= window.__game.renderer.snapshot(), 폴백 canvas.toDataURL). 새 캡처 스크립트는 이걸 복사.
- 반드시 chromium.launch({ headless: false, args:[...swiftshader...] }) (headless면 검은 캔버스). dev config에 render.preserveDrawingBuffer: import.meta.env.DEV 필요(이미 src/main.ts에 있음).
- 캡처 후 python3로 평균밝기 확인(거의 0이면 캡처 실패 = 다시).
