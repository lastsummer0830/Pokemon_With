...
| **"지어내지 않는다"는 원칙을 검증으로 보장** | UI 좌표를 감으로 그리지 않고 원본 Ruby 스크립트(`Scripts.rxdata` → zlib)에서 추출. Hermes 감독 아래 실제 입력·렌더 캡처·픽셀 대조·console·build로 확인(아래 트러블슈팅 3) |
...
| **품질 검증** | Hermes 감독 · 프로젝트 Skill · 실제 게임 입력/렌더 캡처 · console/page error · `npm run build` |
...
### 3. 며칠을 날린 UI 재현 실패 → 원본 근거와 실제 렌더 검증으로 전환
...
- **배운 점 → 검증 계약으로 승격** — 처음에는 편집 전 승인과 종료 차단 hook을 만들었지만, 승인창 남발과 우회 가능한 판정 때문에 신뢰할 수 있는 품질 경계가 되지 못해 폐기했습니다.
  - 현재는 프로젝트 Skill이 원본 근거와 acceptance를 먼저 고정합니다.
  - 구현 뒤 실제 키 입력, Phaser renderer 캡처, 픽셀 대조, scene 상태, console/page error, `npm run build`를 확인합니다.
  - Claude의 완료 보고만 믿지 않고 Hermes가 diff와 실행 증거를 독립 검증한 뒤 완료를 판정합니다.
  - `.claude/.verify/`는 로컬 검증 증거 보관용이며, 파일 존재만으로 통과시키는 강제 hook은 사용하지 않습니다.
...
- **배운 점** — **안전장치는 "있다"가 아니라 "지금 올바른 방향으로 작동한다"를 검증해야 합니다.** 그래서 과거 guard hook은 현재 품질 게이트로 사용하지 않고, 실제 게임 실행과 관찰 가능한 증거를 완료 조건으로 삼습니다.
...
> Git commit hook은 설치하지 않습니다. 커밋은 사용자 승인 대상이며, 그 전에 Hermes가 관련 검사와 `npm run build` 결과를 직접 확인합니다.
