...
- memory `starter-lab-flow` 갱신 필요: **블록1.5 완료(화면비율 cover 확정·기술메뉴 AR화·가방 화살표·배틀 완주 검증) / 다음 = 트레이너전 분리 + 블록2**. → **갱신 완료.**

## 11. 커밋 (`6811cec`) — 위 §1~§7 전부
`battleView.ts` · `BattleScene.ts` · `BagScene.ts` · `data/ar/index.ts` · `types.json` · `extract-battle-data.py` · 신규 `public/assets/ui/types.png` · 이 일지.
커밋 훅(`tsc --noEmit`) 통과.

---

# ▣ 세션 2-b (짧은 마무리) — **대사 조사 하드코딩 제거** (커밋 `f2b6333`)

## A. 왜
대사에 `${name}은(는)`, `${name}이(가)`, `Lv.${lv}(으)로`처럼 **조사를 괄호로 박아넣고 있었다.**
- ⚠️ **이 대사들은 사용자가 쓴 게 아니다**(내가 지어낸 것). "사용자가 설정한 멘트는 안 건드린다" 규칙 때문에 물어봤으나, **사용자 답변: "내가 쓴 거 아니고, 그런 건 당연히 하드코딩하면 안 되는 것."**
- → **앞으로 대사에 조사를 괄호로 박지 말 것. 무조건 `josa()`를 쓴다.**

## B. 무엇을 했나
- **`src/data/josa.ts` 신설** — 조사 함수가 **두 벌로 중복**돼 있었다(`battleView.ts`의 export + `LabScene.ts`의 private 메서드). 둘 다 지우고 **공용 1벌**로 합쳤다(LabScene 쪽이 `과와`·`로`까지 다뤄서 그쪽을 기준으로).
- **`kind` 문자열은 `[받침있음, 받침없음]` 순서**다: `은는` `이가` `을를` `과와` `로`.
- **숫자 뒤 조사도 처리**한다(`Lv.6` 같은 것). 읽는 소리로 받침이 갈린다:
  - 0영·3삼·6육 = 받침 있음 → **으로** / 1일·7칠·8팔 = ㄹ받침 → **로** / 2이·4사·5오·9구 = 받침 없음 → **로**
  - 결과: `Lv.6으로 올랐다` · `Lv.5로 올랐다` · `Lv.1로 올랐다` (전부 맞는 한국어)
- 고친 대사가 있는 곳: **BattleScene**(조우·기술못씀·쓰러뜨림·승부승리·경험치·레벨업·기술습득·쓰러짐) · **BagScene**(못 쓰는 아이템·누구에게 쓸까·기절/회복) · **InteriorScene**(최상 컨디션·눈을 떴다·엄마 대사) · **LabScene**(오박사 대사 — 중복 함수 제거).

## C. 검증
- **배틀 화면(실주행)**: **"상대 구구를 쓰러뜨렸다!"**, **"파이리는 21 경험치를 얻었다!"** (전엔 `구구을(를)`, `파이리은(는)`).
- **가방 화면(실주행 캡처)**: **"상처약을 누구에게 쓸까?"**(받침 ㄱ → 을), **"파이리는 기절해 있다!"**(받침 없음 → 는).
  → 수정 전/후를 나란히 놓은 비교 이미지 = **`.claude/.verify/가방_조사_비교.png`**
- josa 단위 확인: `파이리는`/`리자몽은` · `구구를`/`이상해꽃을` · `개구마르가`/`나오하가` · `네모와`/`웅과` · `5로`/`6으로`/`1로`/`3으로`/`10으로`/`9로`.
- `tsc --noEmit` 통과 · 콘솔 에러 0.
- ⚠️ **`/code-review`는 안 돌렸다**(사용자 퇴근 시간). 다음 세션에 이 커밋(`f2b6333`)도 포함해 한 번 돌릴 것.
- ⚠️ **재검증 못 한 것:** InteriorScene(집)·LabScene(연구소) 대사는 코드만 고치고 **실제로 그 화면을 띄워 보지 않았다.** 조사만 바뀐 문자열 치환이고 배틀·가방 두 화면에서 같은 함수가 정상 동작하는 걸 확인했으니 위험은 낮지만, 다음 세션에 집·연구소를 한 번 띄워 대사가 정상인지 볼 것.

## D. 남은 것 = 위 §9 그대로
1. 트레이너전을 야생전과 분리(+블록2와 같이) → 2. 블록2(교체·상대팀) → 3. 블록3(포획) → 4. 가방·도감·메뉴 와이드 대응
5. ~~조사 하드코딩~~ **완료**.

---

# ▣ 세션 3 (0715 새벽, 세션 연속) — **라이벌 첫 배틀 등·퇴장 흐름 정상화** (커밋 `b540001`)

## A. 왜 (사용자 지적)
스타터 선택 후 "네모가 배틀하자 하면 밖에서 기다리는 게 자연스럽지 않냐 — 계속 안에 있다가 뒤에 나오는 게 아니라." → **코드로 확인하니 실제로 그 버그였다:**
- `LabScene.ts:283` 네모가 "밖에서 기다릴게" 대사만 하고 **연구소 안 제자리에 그대로 서 있었다**(퇴장 연출 없음).
- `WorldScene.ts`(구) `maybeStartRivalBattle` — 마을로 나가면 네모를 **플레이어 위쪽(연구소 방향)에서 걸어 내려오게** 만들었다 = "안에 있다 따라나온" 꼴.

## B. 무엇을 했나 (파일 2개, 둘 다 이번 커밋)
- **`LabScene.ts`**:
  - `NEMONA_EXIT_PATH`(제자리 (11,3)→출구 문 (6,13), **oak_lab.json blocked에 BFS 돌려 뽑은 15칸** — 눈대중 아님) 따라 네모가 한 칸씩 걸어 나가 문에서 사라지는 컷신 `walkNemonaOut()`(await, 문 SFX).
  - 상태 필드 `nemTile`(현재 칸)·`nemGone`(퇴장 여부). `layout()`·`walkable()`이 원위치 대신 이 값을 참조.
  - `nem-*` 걷기 애니(`nemona_ow` 시트, exists 가드).
  - **재입장 처리:** `starterChosen` 레지스트리가 있으면 create에서 `nemGone=true` → 스타터 고른 뒤 연구소 다시 들어가도 네모가 없다.
- **`WorldScene.ts`**:
  - `rivalBattlePending`이면 네모를 **플레이어와 같은 줄 옆 칸(rivalApproach())** 에 미리 세운다(RIVAL_GAP=4칸). 문에서 나오면 '멀찍이 서서 기다리는 네모'가 먼저 보이고, 발견("!")→옆 칸까지 걸어와 대사→배틀.
  - ⚠️ **대기·조우 위치는 고정좌표 금지, 플레이어 기준으로 계산**한다 = `/code-review`에서 잡은 진짜 버그: **라이벌전에서 지면 집(InteriorScene)으로 가고 예약은 남아(이길 때만 소비, BattleScene.ts:259) 집 앞 재대결이 걸리는데**, 네모가 연구소 앞 고정좌표에 있으면 화면 밖에서 혼자 걷고 플레이어만 얼어붙는다. 이제 어디서 재대결해도 화면 안에서 다가온다.
  - `input.enabled=false` 대신 `busy` 플래그만 사용(대사 넘기는 키까지 죽는 것 방지).
  - `scale.off("resize", this.onResize)` 콜백 지정(콜백 없이 부르면 전역 리스너 전부 삭제).
- **조우 대사 2줄을 SV 네모 공식 대사 기반으로 교체**(내가 지어낸 말투 제거 — 사용자 지시로 원본 조사):
  - 1줄 `${you}! 지금 당장 나랑 한판 하자. 네 파트너가 얼마나 굉장한지 보고 싶어!` = Bulbapedia 영문 정본 옮김(**한국어 공식판 미확인**).
  - 2줄 `처음 하는 포켓몬 승부니까 즐기면서 하자!` = **SV 한국어판 verbatim(확인됨)**.
  - ⚠️ **조사 결론:** AR 원본은 라이벌=그린(상록 체육관장), 스타터 직후 배틀 장면 자체가 없음("후딱 끝내버리자고요"는 체육관 대사) → 우리 시나리오(네모, 스타터 직후 마을 앞)엔 안 맞아 **SV 네모 기준 채택.**

## C. 검증 (playwright 실주행 — 전부 통과, 콘솔 에러 0)
스크립트=스크래치패드(리포 밖), 캡처=`.claude/.verify/`.
- **정상 흐름**(D→9 연구소 → 스타터 선택): 네모 퇴장(`nemGone=true`, 마지막 칸 [6,13]) → 마을 진입 시 대기 → 조우 대사 → 배틀 진입. (`r1`~`r7`)
- **재대결 흐름**(예약 남긴 채 집 앞 스폰): 네모 경로 `[[14,8],[15,8],[16,8]]` **화면 안(onScreen=true)** → 대사 → 배틀 진입. (`r8`~`r10`)
- **대사 텍스트 확인**: `dlg.boxText`에서 위 2줄 그대로 출력됨(`r11`).
- `npx tsc --noEmit` 통과(⚠️ **반드시 `myPokemon_AJ/`에서**).
- `/code-review`(high) 7건 → 실제 버그 1건(재대결 고정좌표) + resize 리스너 1건 수정, 대사 규칙위반은 원본 조사로 해소. 나머지(연출용 애니 중복 mkNem 2벌, 걷기 구현 스타일 LabScene↔WorldScene 불일치)는 **미반영 — 다음에 오버월드 캐릭터 헬퍼로 통합할 때 같이.**

## D. 접속 주소 / 직접 볼 곳
**http://localhost:5180** → 타이틀에서 **D → 9**(연구소) → 스타터 선택하고 대사 넘기면 네모 퇴장 → 아래 문으로 나가면 마을에서 조우.

## E. 다음 세션 시작 지점
- **블록1-B = 파티 교체(switch) 실동작** (사용자 승인 완료, 이번엔 시간상 안 함). `BattleScene`의 `switch` 커맨드는 아직 `"지금은 포켓몬을 교체할 수 없다!"` 스텁. 함정: 스프라이트 텍스처는 `preload()`에서만 로드 → 런타임 `load.image`+`textures.remove()` 필요. `ally`가 registry 파티 선두와 같은 참조라는 전제(경험치·회복). → 그 다음이 §9의 트레이너전 분리+블록2.
- 미반영 리뷰 2건(mkNem 중복, 걷기 구현 통일)은 오버월드 캐릭터 컷신 헬퍼 만들 때.

## F. 새 지침/skills/memory 요지
- **AGENTS.md 승격 후보(대사 규칙 재확인):** 신규 대사는 반드시 공식 대사 조사 후. 우리 라이벌=**SV 네모**(AR 그린 아님) — AR엔 스타터 직후 라이벌전이 없다.
- memory `starter-lab-flow` 갱신 필요: **라이벌 등·퇴장 흐름 완성** / 다음 = 블록1-B(파티 교체).
