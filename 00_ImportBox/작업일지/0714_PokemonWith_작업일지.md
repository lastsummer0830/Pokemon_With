...
## C. 검증
- 실제 배틀 주행 대사: **"상대 구구를 쓰러뜨렸다!"**, **"파이리는 21 경험치를 얻었다!"** (전엔 `구구을(를)`, `파이리은(는)`).
...
- ⚠️ **재검증 못 한 것:** InteriorScene(집)·LabScene(연구소) 대사는 코드만 고치고 **실제로 그 화면을 띄워 보지 않았다.** 조사만 바뀐 문자열 치환이라 위험은 낮지만, 다음 세션에 집·연구소를 한 번 띄워 대사가 정상인지 볼 것.
