...
## 6.5 장면 확인 · 디자인 결정 워크플로 (사용자 확정 — 항상 적용)
> 사용자가 결과를 쉽게 확인·결정하게 하는 두 규칙. 새 기능 만들 때마다 지킨다.
- **주요 씬은 `DebugMenuScene`에 등록한다.** 새로 만든 씬(메뉴·파티·가방·박스·배틀 등)은 타이틀 **D키 → DebugMenuScene**에서 숫자키로 바로 진입할 수 있게 항목을 추가한다. (사용자가 매번 인트로/이동 안 거치고 그 화면만 바로 확인) → `src/scenes/DebugMenuScene.ts`.
- **UI 생김새(look)가 갈리는 건 임의 확정 말고 Pick 미리보기로 사용자가 고른다.** 가방·저장/불러오기·인게임 메뉴·박스 등 "어떻게 생겼나"가 중요한 화면은 **2~3개 스타일 변형을 playwright 실제 스크린샷으로 찍어 `01_Resources/Pick/<카테고리>/`에 넣고(+비교 몽타주) 사용자에게 "골라달라"고 요청**한다. 고른 뒤에만 그 look으로 확정. (§3 ⭐후보 규칙과 동일 철학 — 기능은 내가 만들되 look은 사용자 결정.)
