...
- **화풍 = 픽셀(도트) 전용.** 매끈한 일러스트(공식 아트워크·PokeAPI HOME 512 등)는 인게임 에셋 **금지.** "고화질/최품질"이라 해도 = **"최고 품질의 픽셀"**(일러스트 아님).
  - **사용자가 넣은 에셋은 화풍을 이유로 임의 교체하지 말 것.**
...
- **난이도는 '노말' 하나뿐** (2026-08-02 확정). 원본은 시작할 때 이지/노말/하드/익스트림/인새인 5개를 고르게 하지만(Map072 이벤트) **우리는 선택 화면을 만들지 않는다.** `systems/difficulty.ts`의 5종 표와 세이브의 `difficulty` 필드는 그대로 두되(원본 실측 근거·세이브 호환), **"선택 화면이 없네?" 하고 다시 만들지 말 것.**
...
- **⛔ 사용자에게 보여줄 사진을 `.claude/.verify/`에 넣지 말 것** (2026-08-02, 같은 지적 2회 — 두 폴더의 용도가 다르다):
  - `.claude/.verify/` = **자동 검증이 남기는 원본 프레임**(훅 `enforce-ui-verify.sh`가 요구하는 증거). 기계용이다.
  - `01_Resources/Pick/<카테고리>/` = **사용자가 눈으로 보고 판단할 것** — 비교 사진·몽타주·후보·"이거 어때?"용 화면 캡처는 **전부 여기**.
  - 채팅에서 "○○.png 열어봐라"라고 경로를 대는 사진은 **무조건 Pick**이다.
...
- **⭐ 새 작업 = 확인 항목을 디버그 페이지에 추가하는 것까지가 '완료'다** (2026-07-22 사용자 지시. 사용자는 학원 때문에 **하루 한 번 몰아서** 확인한다 — 매번 상황을 손으로 만들게 하면 안 된다).
  - 추가 위치 = **`src/data/debugChecks.ts`의 `DEBUG_CHECKS`**. 항목 하나 = `date`(작업일지 MMDD) + `title` + `what`(뭘 고쳤나 한 줄) + `see`(화면에서 뭘 보나 한 줄) + **그 연출로 한 방에 가는 `scene`/`data`**.
  - 연출이 배틀 안에서만 나오는 것(애니·잔뎀·회복 등)은 `BattleScene`의 **데모 모드**(`demo: move|common|residual|msgbox`)를 쓰거나 같은 방식으로 하나 더 만든다 — 배틀을 진행시키지 않고 그 연출만 재생한다.
  - 실행하면 **상단 확인바**(`DebugCheckBarScene`)가 떠서 `[`이전 · `]`다음 · `\`목록으로 **하루치를 순서대로** 훑는다.
  - 검증 스크립트: `node tools/shot-debugcheck.mjs <출력폴더>`(headless, tools/ 안에서 실행).
...
### ⭐ 세션 마무리 정형 (2026-07-22 사용자 지시 — 매번 이 순서, 중간에 되묻지 말 것)
**① 작업일지 작성 → ② 커밋 → ③ push → ④ 다음 세션 첫 프롬프트를 채팅으로 직접 출력.**
- **"작업일지 써"는 커밋·push까지 포함한 지시다.** 이 흐름에서만 커밋·push 별도 승인을 안 받는다(그 외 커밋·push는 여전히 승인 필요).
- ④는 일지에 적는 것만으론 부족하다 — **채팅 마지막에 복붙 가능한 형태로** 낸다(사용자가 `/clear` 직후 그대로 쓴다).
