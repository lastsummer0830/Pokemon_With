...
const BUILDINGS: Record<Building, BuildingDef> = {
  pc: {
    ...
    imgKey: "bld_pc_map", jsonKey: "bld_pc_col",
  ...
  mart: {
    ...
    imgKey: "bld_mart_map", jsonKey: "bld_mart_col",
    bgmKey: BGM.mart, bgmFile: "assets/audio/bgm_mart.ogg",
    spawn: [4, 7], toCity: [35, 26],
    npcs: [
      { key: "npc_clerk", file: "assets/characters/NPC 19.png", tile: [2, 3], face: "down" },
    ],
...
export default class BuildingScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const texKeys = [d.imgKey, this.texKey, ...d.npcs.map((n) => n.key)];
  ...
  private layout(): void {
    ...
    const src = this.textures.get(this.def.imgKey).getSourceImage();
  ...
  private async healParty(): Promise<void> {
    this.busy = true;
    await this.dlg.say("어서 오세요! 포켓몬 센터에 잘 오셨습니다.", "간호순");
    await this.dlg.say("포켓몬을 건강한 상태로 되돌려 드릴까요?", "간호순");
    const yes = await this.dlg.askYesNo();
    if (!yes) {
      await this.dlg.say("네, 다음에 또 들러 주세요!", "간호순");
    ...
    await this.dlg.say("그럼 포켓몬을 맡아 주세요.", "간호순");
    this.dlg.hide();

    // 회복 연출 — 화면을 잠깐 어둡게 하고 회복 챠임(전용 파일이 없어 획득 팡파레로 대체 — 체육관 배지와 같은 처리).
    await this.fade(true);
    playSfx(this, SFX.pkmnGet, 0.5);
    ...
    await this.dlg.say("오래 기다리셨습니다! 포켓몬이 모두 건강해졌습니다.", "간호순");
    await this.dlg.say("포켓몬과 함께 늘 힘내세요. 다녀오세요!", "간호순");
  ...
  private async clerkGreet(): Promise<void> {
    this.busy = true;
    await this.dlg.say("어서 오세요! 프렌들리 숍입니다.", "점원");
    await this.dlg.say("무엇을 찾으시나요?", "점원");
