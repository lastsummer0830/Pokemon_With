...
// ★ 집에서 쉬기 — 유대(bond)를 올리는 여러 '케어' 중 하나 (순수 함수 모음).
//   유대 개념·값·배틀보정의 단일 원천은 systems/bond.ts. 여기선 '집에서 재우면 얼마나 오르나'만 계산한다.
//
//  규칙(한눈에):
//   - 방을 어떻게 꾸몄는지가 그 방에서 도달 가능한 **유대 상한**을 정한다. (빈 방이면 상한 20)
//   - 침대에서 **잠을 잘 때마다** 유대가 그 상한까지 조금씩 오른다. (1회 +10)
//   - 유대는 배틀에서 데미지 배율로 읽힌다(systems/bond.ts → battle.ts: 100이면 +10%).
//  즉 "잘 꾸민 방 = 더 높은 상한 = 더 강한 포켓몬"이고, 잠만 자서는 상한을 못 넘는다.
//  (유대는 쓰다듬기 등 다른 케어로도 오르지만, 그 방의 잠자기 상한과는 별개다.)
