...
| **Another Red** (사용자 제공 또는 현재 환경에서 확인한 로컬 source) | 로컬 | 9세대 포함 애니 Front 시트(대문자 파일명) | ⭐ 9세대 픽셀 핵심. 경로를 추정하지 말고 확인 후 `tools/import-from-anotherred.mjs`로 필요한 파일만 복사 |
...
| **Another Red Tilesets** (사용자 제공 또는 확인된 로컬 source) | 로컬 | 마을 맵 타일셋 | 필요한 파일만 `public/assets/tilesets/`에 이식; 이름의 공백·유니코드 확인 |
...
## E. 사운드 (`public/assets/audio/`에 OGG 사용 중; 추가 전 실제 inventory 확인)
| 소스 | 용도 | 비고 |
|---|---|---|
| **Another Red Audio** (사용자 제공 또는 확인된 로컬 source) | BGM/효과음 | 톤 일치 후보; 파일별 출처와 사용 범위를 확인 |
